from models.fractal import FractalConfig, JuliaConfig, MandelbrotConfig
...
def render_julia(config: JuliaConfig, real, imag):
    z = real + 1j * imag
...
def render(config: FractalConfig, render_func: callable) -> np.ndarray:
    if type(config) == JuliaConfig:
        bound_x_min = -1.5
        bound_x_max = 1.5
    elif type(config) == MandelbrotConfig:
        bound_x_min = -2.0
        bound_x_max = 1.0
    bound_y_min = -1.5
    bound_y_max = 1.5
